...
class DataSeeder:
    ...
    def _generate_signal_events(self, base_time: datetime, count: int = 10) -> List[Dict]:
    ...
    def _generate_trade_events(self, base_time: datetime, count: int = 5) -> List[Dict]:
        ...
        for i in range(count):
            ...
            close_ts = (base_time + timedelta(hours=i*2, seconds=duration_sec)).isoformat() + 'Z'
    ...
    def _generate_telegram_events(self, base_time: datetime, count: int = 12) -> List[Dict]:
...
def seed_test_data_cli():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "today":
            ...
            confirm = input("⚠️  TÜM log dosyaları silinecek! Onaylıyor musunuz? (yes/no): ")
            ...
            print("Kullanım: python seed_data.py [today|week|minimal|cleanup|cleanup-all]")
    else:
        # Default: seed today
        seeder.seed_today(full=True)
