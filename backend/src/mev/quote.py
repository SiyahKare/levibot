...
async def _http_json(
    url: str, headers: dict | None = None, params: dict | None = None, timeout=8
) -> dict:
...
async def quote_0x(
    chain: str, sell: str, buy: str, sell_amount_wei: int
) -> dict | None:
...
async def quote_symbol(
    chain: str, sell_sym: str, buy_sym: str, sell_amount: float
) -> dict:
    addr = TOKENS.get(chain, TOKENS["ethereum"])
    sell = addr.get(sell_sym.upper())
    buy = addr.get(buy_sym.upper())
    ...
    if j:
        return {
            "ok": True,
            "price": float(j["price"]),
            "buyAmount": j.get("buyAmount"),
            "sources": j.get("sources"),
        }
    # offline fallback: basit sentetik fiyat
    # price ~ 2000 (ETH→USDC), diğerleri 1.0
    base_price = (
        2000.0 if sell_sym.upper() == "ETH" and buy_sym.upper() == "USDC" else 1.0
    )
