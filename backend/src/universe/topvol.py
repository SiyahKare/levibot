...
class TopVolTracker:
    ...
    def __init__(self, w_15m: float = 0.6, w_1h: float = 0.3, w_rvol: float = 0.1):
        """
    ...
    def get_top_n(self, n: int = 12) -> list[VolumeMetrics]:
        ...
        sorted_metrics = sorted(
            self._metrics.values(), key=lambda m: m.score, reverse=True
        )
...
async def compute_volume_metrics_from_db(
    symbols: list[str], window: Literal["15m", "1h"] = "15m"
