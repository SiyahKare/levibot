...
class TradingEngine:
    ...
    async def stop(self, timeout: float = 10.0) -> None:
        ...
        if self._task:
            try:
                ...
                self.logger.warning(f"Engine {self.symbol} stop timeout, cancelling task")
                self._task.cancel()
                try:
                    await self._task
    ...
    def _init_ml_components(self) -> None:
        ...
        provider = GemmaSentimentProvider(
            rpm=sentiment_cfg.get("rpm", 60)
        )
        self._sentiment = SentimentExtractor(
            provider=provider,
            ttl_seconds=sentiment_cfg.get("ttl", 900)
        )
        ...
        self._onchain = OnchainFetcher(
            ...
            ttl_seconds=onchain_cfg.get("ttl", 300)
        )
    ...
    async def _generate_signal(self, market_data: dict) -> dict | None:
        ...
        try:
            ...
            sentiment = await self._sentiment.score(
                f"{self.symbol}:latest",
                texts
            )
    ...
    def _check_risk(self, signal: dict | None) -> bool:
        ...
        if self.risk.is_global_stop():
            self.logger.warning(f"Global stop active (daily loss: {self.risk.realized_today_pct():.2f}%), skipping order")
            return False
        
        # Concurrent positions check
        if not self.risk.can_open_new_position(self.symbol):
            self.logger.info(f"Max concurrent positions ({self.risk.policy.max_concurrent_positions}) reached, skipping order")
            return False
