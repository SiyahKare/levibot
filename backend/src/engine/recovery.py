...
class RecoveryPolicy:
    ...
    def should_recover(self, symbol: str) -> bool:
        ...
        recent_restarts = [
            ts for ts in self.restart_history[symbol] if ts > one_hour_ago
        ]
        ...
        if len(recent_restarts) >= self.max_restarts_per_hour:
            print(
                f"❌ Max restarts ({self.max_restarts_per_hour}/hour) reached for {symbol}"
            )
            return False
        ...
        if recent_restarts:
            ...
            if elapsed < min_wait:
                print(
                    f"⏳ Backoff active for {symbol}: {int(min_wait - elapsed)}s remaining"
                )
                return False
