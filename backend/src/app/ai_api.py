...
from ..ai.openai_client import explain_anomaly, regime_advice, score_headline, score_headlines
...
async def ai_news_score_batch(req: NewsScoreBatchRequest) -> dict[str, Any]:
    ...
    try:
        ...
        log_event("AI_NEWS_SCORE_BATCH", {"count": len(req.headlines), "results": results})
        return {"ok": True, "results": results}
    except ValueError as e:
        raise HTTPException(status_code=503, detail=f"OpenAI service unavailable: {str(e)}")
    except Exception as e:
        log_event("ERROR", {"scope": "ai_news_score_batch", "error": str(e)})
        raise HTTPException(status_code=500, detail=f"Batch news scoring failed: {str(e)}")
