...
def _current_model_name() -> str:
    ...
    try:
        flags = load_flags()
        return (flags.get("ai_model") or "stub-sine") if isinstance(flags, dict) else "stub-sine"
    except Exception:
        return "stub-sine"
...
async def predict(
    ...
    model: str | None = Query(None, description="Model name (optional, uses current if not specified)")
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        PREDICTION_REQUESTS.labels(model_name=model_name, symbol=symbol, status=status).inc()
        
        if result.get("fallback"):
            FALLBACK_EVENTS.labels(reason=result.get("fallback_reason", "unknown")).inc()
