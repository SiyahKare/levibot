...
def lse_health(symbol: str = "BTCUSDT"):
    ...
    return {
        ...
        "guards": engine_health.get(
            "guards",
            {"vol_ok": True, "spread_ok": True, "latency_ok": True, "risk_ok": True},
        ),
...
def lse_run(req: RunRequest):
    ...
    if req.running is True:
        ...
        audit(
            "lse.run.start",
            {"mode": req.mode, "symbol": symbol, "overrides": req.overrides},
        )
        return {
            "ok": True,
            "running": True,
    ...
    if req.overrides:
        ...
        return {"ok": True, "message": "Overrides applied", "symbol": symbol}

    raise HTTPException(
        status_code=400, detail="running must be true or false, or provide overrides"
    )
...
def lse_run_batch(req: BatchRunRequest):
    ...
    audit(
        "lse.run_batch",
        {"symbols": req.symbols, "action": req.action, "mode": req.mode},
    )
...
def lse_pnl_stats(hours: int = 24):
    ...
    try:
        ...
        stats = TradesRepository.get_pnl_stats(
            strategy="lse", symbol=ENGINE._cfg.symbol, hours=hours
        )
        ...
        try:
            ...
            return {
                ...
                "win_rate": (
                    pnl_data.get("win_rate", 0.0) / 100.0
                    if pnl_data.get("win_rate")
                    else 0.0
                ),
                "equity_curve": [],
            }
