...
try:
    ...
    def alert_cooldown_triggered(*args, **kwargs):
        pass

    def alert_cooldown_cleared(*args, **kwargs):
        pass

    def alert_guardrails_rejection(*args, **kwargs):
        pass
...
def list_presets() -> dict[str, Any]:
    ...
    """
    return {"ok": True, "presets": PRESETS, "current": _CURRENT_PRESET}


@router.post("/preset/{name}")
def set_preset(
    name: str, req: Request = None, _: bool = Depends(require_admin)
) -> dict[str, Any]:
    """
    Apply a risk preset.
    ...
    audit(
        "risk_preset",
        {"name": name, "ip": req.client.host if req and req.client else "unknown"},
    )
...
def update_guardrails(
    payload: dict[str, Any], req: Request = None, _: bool = Depends(require_admin)
) -> dict[str, Any]:
    """
    ...
    if "max_trade_usd" in payload:
        ...
        if val <= 0:
            raise HTTPException(
                status_code=400, detail="max_trade_usd must be positive"
            )
    ...
    if "cooldown_minutes" in payload:
        ...
        if val < 0:
            raise HTTPException(
                status_code=400, detail="cooldown_minutes must be non-negative"
            )
    ...
    if "circuit_breaker_latency_ms" in payload:
        ...
        if val < 0:
            raise HTTPException(
                status_code=400,
                detail="circuit_breaker_latency_ms must be non-negative",
            )
    ...
    if "symbol_allowlist" in payload:
        ...
        if not isinstance(allowlist, list):
            raise HTTPException(
                status_code=400, detail="symbol_allowlist must be a list"
            )
...
def trigger_cooldown(
    ...
    _COOLDOWN_UNTIL = datetime.utcnow() + timedelta(
        minutes=_GUARDRAILS.cooldown_minutes
    )
...
def clear_cooldown(
    ...
    # Audit log
    audit(
        "cooldown_clear", {"ip": req.client.host if req and req.client else "unknown"}
    )

    # Telegram alert
    alert_cooldown_cleared()

    return {"ok": True, "cooldown_active": False, "message": "Cooldown cleared"}
