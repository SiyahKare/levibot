...
async def _get_model_prediction(symbol: str, horizon: str = "60s") -> dict[str, Any]:
    ...
    try:
        ...
        from ...infra.ml_metrics import ml_feature_staleness_seconds, ml_prediction_confidence
        ...
        if result.get("staleness_s") is not None:
            ml_feature_staleness_seconds.labels(symbol=symbol).set(result["staleness_s"])
