...
def _get_paper_stats() -> dict[str, Any]:
    ...
    try:
        ...
        trades_today = sum(
            1 for trade in portfolio.trade_history
...
async def get_automation_status():
    ...
    try:
        ...
        stop_loss_pct = abs(flags.get("max_daily_loss", -200.0)) / 10000.0  # Convert to fraction
        take_profit_pct = 0.03  # 3% default
        
        # Policy thresholds
        policy = {
            "entry": confidence_threshold,
            "exit": confidence_threshold - 0.2
        }
...
async def start_automation(
    canary_fraction: float = Body(default=0.0),
    min_conf: float = Body(default=0.6)
):
