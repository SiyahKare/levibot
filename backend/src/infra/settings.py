...
class Settings:
    ...
    AI_REASON_MONTHLY_TOKEN_BUDGET = int(os.getenv("AI_REASON_MONTHLY_TOKEN_BUDGET", "250000"))  # ~250k tokens/month
    ...
    # --- Data Persistence ---
    DATA_DIR = os.getenv("DATA_DIR", str(Path(__file__).resolve().parents[3] / "backend" / "data"))
    ...
    ALLOW_IPS = [ip.strip() for ip in os.getenv("ALLOW_IPS", "").split(",") if ip.strip()]
    ...
    IP_ALLOWLIST = [ip.strip() for ip in os.getenv("IP_ALLOWLIST", "127.0.0.1,::1").split(",") if ip.strip()]
