...
class AuditLogger:
    ...
    def __init__(self, redis_url: str, event_bus: EventBus | None = None):
...
async def audit_login(
    logger: AuditLogger, user_id: str, success: bool, ip_address: str | None = None
):
    """Audit user login"""
    await logger.log(
...
async def audit_kill_switch(
    logger: AuditLogger, activated: bool, reason: str, user_id: str | None = None
):
    ...
    await logger.log(
        ...
        details={"activated": activated, "reason": reason},
        user_id=user_id,
    )


async def audit_model_deployment(
    logger: AuditLogger, model_version: str, stage: str, user_id: str | None = None
):
    ...
    await logger.log(
        ...
        details={"version": model_version, "stage": stage},
        user_id=user_id,
    )


async def audit_compliance_check(
    logger: AuditLogger, check_type: str, passed: bool, details: dict | None = None
):
    ...
    await logger.log(
        ...
        details={"check_type": check_type, "passed": passed, **(details or {})},
    )
...
def get_audit_logger(
    redis_url: str = "redis://localhost:6379/0", event_bus: EventBus | None = None
) -> AuditLogger:
