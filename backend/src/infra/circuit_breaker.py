...
class CircuitBreaker:
    ...
    def __init__(self, name: str, redis_url: str, config: CircuitConfig | None = None):
        self.name = name
...
class CircuitBreakerManager:
    ...
    def register(
        ...
        if name not in self._breakers:
            self._breakers[name] = CircuitBreaker(name, self.redis_url, config)
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        exchange_cb = manager.register(
            "exchange_api", CircuitConfig(failure_threshold=3, timeout_seconds=30)
        )

        db_cb = manager.register(
            "database", CircuitConfig(failure_threshold=5, timeout_seconds=60)
        )
