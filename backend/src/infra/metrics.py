...
signals_generated_total = Counter(
    "levibot_signals_generated_total", "Total signals generated", ["strategy", "symbol"]
)
...
orders_filled_total = Counter(
    "levibot_orders_filled_total", "Total orders filled", ["symbol", "side"]
)

orders_rejected_total = Counter(
    "levibot_orders_rejected_total", "Total orders rejected", ["symbol", "reason"]
)

# Trades
trades_total = Counter(
    "levibot_trades_total", "Total trades completed", ["strategy", "symbol", "outcome"]
)
...
portfolio_equity_usd = Gauge(
    "levibot_portfolio_equity_usd", "Current portfolio equity in USD"
)

portfolio_cash_usd = Gauge("levibot_portfolio_cash_usd", "Current cash balance in USD")

portfolio_unrealized_pnl_usd = Gauge(
    "levibot_portfolio_unrealized_pnl_usd", "Unrealized PnL in USD"
)

portfolio_realized_pnl_usd = Gauge(
    "levibot_portfolio_realized_pnl_usd", "Realized PnL in USD"
)

portfolio_exposure_pct = Gauge(
    "levibot_portfolio_exposure_pct", "Portfolio exposure as percentage of equity"
)

portfolio_num_positions = Gauge(
    "levibot_portfolio_num_positions", "Number of open positions"
)

portfolio_daily_pnl_usd = Gauge("levibot_portfolio_daily_pnl_usd", "Daily PnL in USD")

portfolio_daily_return_pct = Gauge(
    "levibot_portfolio_daily_return_pct", "Daily return percentage"
)

portfolio_drawdown_pct = Gauge(
    "levibot_portfolio_drawdown_pct", "Current drawdown percentage"
)
...
feature_store_cache_hits = Counter(
    "levibot_feature_store_cache_hits", "Feature store cache hits", ["symbol"]
)

feature_store_cache_misses = Counter(
    "levibot_feature_store_cache_misses", "Feature store cache misses", ["symbol"]
)
...
policy_evaluations_total = Counter(
    "levibot_policy_evaluations_total", "Total policy evaluations", ["decision"]
)

policy_cooldown_active = Gauge(
    "levibot_policy_cooldown_active", "Whether cooldown is active", ["symbol"]
)

policy_kill_switch_active = Gauge(
    "levibot_policy_kill_switch_active", "Whether kill switch is active"
)

policy_daily_trades_count = Gauge(
    "levibot_policy_daily_trades_count", "Number of trades today"
)

policy_daily_loss_limit_remaining_usd = Gauge(
    "levibot_policy_daily_loss_limit_remaining_usd", "Remaining daily loss limit in USD"
)
...
build_info = Info("levibot_build", "Build information")
...
def track_signal_executed(strategy: str, symbol: str, side: int):
    ...
    signals_executed_total.labels(
        strategy=strategy, symbol=symbol, side=side_label
    ).inc()


def track_signal_blocked(strategy: str, symbol: str, reason: str):
    """Track signal blocked by policy"""
    signals_blocked_total.labels(strategy=strategy, symbol=symbol, reason=reason).inc()


def track_trade_completed(
    strategy: str, symbol: str, pnl: float, duration_seconds: float, outcome: str
):
    """Track completed trade"""
    trades_total.labels(strategy=strategy, symbol=symbol, outcome=outcome).inc()

    trade_pnl_usd.labels(strategy=strategy, symbol=symbol).observe(pnl)
    trade_duration_seconds.labels(strategy=strategy, symbol=symbol).observe(
        duration_seconds
    )
...
def update_policy_metrics(
    kill_switch_active: bool, daily_trades: int, daily_loss_limit_remaining: float
):
...
def set_build_info(version: str, commit: str, branch: str):
    ...
    build_info.info({"version": version, "commit": commit, "branch": branch})
