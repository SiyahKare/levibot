...
engine_uptime = Gauge(
    "levi_engine_uptime_seconds", "Engine uptime in seconds", ["symbol"]
)

engine_heartbeat = Gauge(
    "levi_engine_last_heartbeat", "Last heartbeat timestamp (epoch)", ["symbol"]
)

engine_errors = Counter(
    "levi_engine_errors_total", "Total engine cycle errors", ["symbol"]
)

engine_status = Gauge(
    "levi_engine_status", "Engine status: 0=stopped, 1=running, 2=crashed", ["symbol"]
)
...
signal_prob = Gauge(
    "levi_signal_prob_up", "Ensemble probability of price increase (0-1)", ["symbol"]
)

signal_conf = Gauge(
    "levi_signal_confidence", "Ensemble confidence score (0-1)", ["symbol"]
)
...
equity_now = Gauge("levi_equity_now", "Current total equity")

realized_today_pct = Gauge(
    "levi_realized_today_pct", "Realized PnL today as percentage"
)

positions_open = Gauge("levi_positions_open", "Number of open positions")

global_stop = Gauge("levi_global_stop", "Global stop loss active: 1=active, 0=inactive")
