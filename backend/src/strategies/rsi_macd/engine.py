...
class RsiMacdEngine:
    ...
    def __init__(self, config: RsiMacdConfig, mode: Literal["paper", "live"] = "paper"):
        ...
        logger.info(
            f"RsiMacdEngine initialized: mode={mode}, symbol={config.symbol}, tf={config.tf}"
        )
    ...
    def _check_entry(
        self, price: float, features: dict, current_bar: int
    ) -> dict | None:
        """
        ...
        # Sync check
        sync_ok = (
            features["sync_bars_since_rsi"] <= self._cfg.sync_bars
            and features["sync_bars_since_macd"] <= self._cfg.sync_bars
        )
    ...
    def _execute_entry(self, price: float, side: str, features: dict):
        ...
        logger.info(
            f"ENTRY {side.upper()}: price={price:.2f}, sl={sl_price:.2f}, tp={tp_price:.2f}"
        )
    ...
    def _execute_exit(self, price: float, reason: str):
        ...
        logger.info(
            f"EXIT {side.upper()}: price={price:.2f}, pnl={net_pnl:.2f}, reason={reason}"
        )
