...
class PolicyEngine:
    ...
    async def record_trade(
        ...
        # Set cooldown
        cooldown_duration = (
            self.limits.cooldown_after_sl_sec if hit_sl
            else self.limits.cooldown_after_loss_sec if pnl < 0
            else self.limits.min_time_between_trades_sec
        )
        
        cooldown_until = time.time() + cooldown_duration
        await self._client.set(
            f"policy:cooldown:{symbol}",
            cooldown_until,
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        print(f"   Daily trades: {status['daily_trades']}/{status['limits']['max_daily_trades']}")
