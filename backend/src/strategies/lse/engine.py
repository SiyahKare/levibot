...
class LSEEngine(StrategyEngine):
            ...
            if self._executor:
                ...
                if self._executor.has_position():
                    ...
                    sl_result = self._executor.check_stop_loss(
                        current_price, sl_pct=sl_bps / 10000.0
                    )
                    if sl_result:
                    ...
                    tp_result = self._executor.check_take_profit(
                        current_price, tp_pct=tp_bps / 10000.0
                    )
                    if tp_result:
    ...
    def _calculate_features(self, ts: float):
        ...
        zscore = calculate_zscore(
            self._mock_prices, lookback=min(120, len(self._mock_prices))
        )
    ...
    def pnl(self) -> dict:
        ...
        return {"realized_pnl": 0.0, "trades": 0}
