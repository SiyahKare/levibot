...
async def run_dispatcher() -> None:
    ...
    if not r:
        print(
            "[Dispatcher] ERROR: Redis not configured. Set REDIS_URL environment variable."
        )
        return

    print(f"[Dispatcher] Connected to Redis: {settings.REDIS_URL}")
    print(
        f"[Dispatcher] Consuming streams: {settings.STREAM_TOPIC_TICKS}, {settings.STREAM_TOPIC_SIGNALS}"
    )

    # Start consumers in parallel
    await asyncio.gather(
        consume_stream(
            settings.STREAM_TOPIC_TICKS, handle_tick, batch_size=500, block_ms=1000
        ),
        consume_stream(
            settings.STREAM_TOPIC_SIGNALS, handle_signal, batch_size=100, block_ms=1000
        ),
    )
