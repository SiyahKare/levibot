...
def trend_score(symbol: str, timeframe: str = "1m", fast: int = 21, slow: int = 55, adx_len: int = 14) -> float:
    path = f"backend/data/parquet/ohlcv/{symbol}_{timeframe}.parquet"
    df = d.sql(f"SELECT close, high, low FROM read_parquet('{path}') ORDER BY time").df()
    if df.empty:
        return 0.5
    ...
    up[0] = 0
    dn[0] = 0
    pdi = 100 * np.convolve(up, np.ones(adx_len) / adx_len, "same") / np.maximum(tr, 1e-9)
    ndi = 100 * np.convolve(dn, np.ones(adx_len) / adx_len, "same") / np.maximum(tr, 1e-9)
