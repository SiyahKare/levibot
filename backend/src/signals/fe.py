...
KNOWN_SYMBOLS = {
    "BTC", "ETH", "SOL", "AVAX", "MATIC", "BNB", "ADA", "DOT", "LINK", "UNI",
    "ATOM", "LTC", "XRP", "DOGE", "SHIB", "APT", "ARB", "OP", "SUI", "SEI",
    "INJ", "TIA", "PEPE", "WIF", "BONK", "FET", "RNDR", "IMX", "NEAR", "FIL"
}
...
NUM = r"(\d+(?:\.\d+)?)"
RE_TP = re.compile(r"\b(?:tp|t/p|take[\s-]?profit|target)(?:\s+at)?\s*[:=]?\s*" + NUM, re.I)
RE_SL = re.compile(r"\b(?:sl|s/l|stop[\s-]?loss)(?:\s+at)?\s*[:=]?\s*" + NUM, re.I)

# SIZE: qty 0.5, size 100, notional$ 25, risk 20usd
RE_SIZE = re.compile(r"\b(?:size|qty|quantity|notional|risk)\s*[:=]?\s*"+NUM+r"(?:\s*(?:usd|\$|usdt|dollar|dollars))?", re.I)
...
def parse_features(text: str) -> Dict[str, Any]:
    ...
    if mtp: out["tp"] = float(mtp.group(1))
    if msl: out["sl"] = float(msl.group(1))
    if msz: out["size"] = float(msz.group(1))
