...
from .precision import MarketMeta, quantize_price, quantize_amount, passes_min_notional
...
class ExchangeRouter:
    ...
    def _make_client(self, exchange: str, testnet: bool):
        if exchange == "bybit":
            ...
            api_secret = os.getenv("LEVI_ONUR_BYBIT_SECRET") or os.getenv("BYBIT_API_SECRET", "")
            ...
            client = ccxt.bybit({"apiKey": api_key, "secret": api_secret, "options": options})
            client.set_sandbox_mode(testnet)
            return client
        if exchange == "binance":
            api_key = os.getenv("LEVI_ONUR_BINANCE_KEY") or os.getenv("BINANCE_API_KEY", "")
            api_secret = os.getenv("LEVI_ONUR_BINANCE_SECRET") or os.getenv("BINANCE_API_SECRET", "")
            client = ccxt.binance({"apiKey": api_key, "secret": api_secret, "options": {"defaultType": "future"}})
            if testnet:
                # Override futures testnet endpoints
                try:
                    client.urls["api"]["fapiPublic"] = "https://testnet.binancefuture.com/fapi/v1"
                    client.urls["api"]["fapiPrivate"] = "https://testnet.binancefuture.com/fapi/v1"
                    client.urls["api"]["fapiPrivateV2"] = "https://testnet.binancefuture.com/fapi/v2"
                except Exception:
                    pass
            client.set_sandbox_mode(testnet)
            return client
    ...
    def _norm_order_inputs(self, symbol: str, side: str, qty: float, price: float) -> tuple[float, float]:
    ...
    def place_oco(self, symbol: str, side: str, amount: float, entry: float, tp: float, sl: float) -> OrderResult:
        ...
        try:
            entry_order = self.client.create_order(symbol, "limit", side, qa, qp, params)
        ...
        tp_order = self.client.create_order(symbol, "limit", hedge_side, qa, tp, {"reduceOnly": True})
        sl_order = self.client.create_order(symbol, "stop", hedge_side, qa, None, {"stopPrice": sl, "reduceOnly": True})
        self.logger.write("OCO_ARMED", {"symbol": symbol, "tp": tp, "sl": sl, "qty": amount})
        return OrderResult(id=str(entry_order.get('id') if entry_order else 'entry_twap'), status="placed", price=entry)

    # Example entry helper: open using sizing with pulse
    def open_with_sizing(self, ccxt_symbol: str, side: str, entry: float, tp: float, sl: float, mark: float, pulse_factor: float, risk_cfg: dict) -> dict:
        ...
        rp = RiskParams(
            ...
            max_pos_notional_pct=(float(risk_cfg.get("max_pos_notional_pct"))/100.0 if risk_cfg.get("max_pos_notional_pct") is not None else None),
            max_pos_usd=(float(risk_cfg.get("max_pos_usd")) if risk_cfg.get("max_pos_usd") is not None else None),
            hard_cap=float(app_cfg.load_features_config().get("telegram", {}).get("evaluation", {}).get("pulse", {}).get("hard_cap", 1.5)),
        )
