...
class MexcExecutor:
    ...
    def __init__(
        ...
        if sandbox:
            try:
                ...
                log_event("MEXC_SANDBOX_ENABLED", {"warning": "MEXC sandbox may have limited support"})
    ...
    def normalize_symbol(self, symbol: str) -> str:
        ...
        raise ValueError(f"Unknown symbol for MEXC: {symbol}. Available: {list(markets.keys())[:10]}...")
    ...
    def place_market_order(
        ...
        try:
            ...
            if not ok:
                log_event(
                    "ORDER_BLOCKED",
                    {"reason": reason, "symbol": norm_sym, "side": side, "notional": notional},
                    symbol=base_sym,
                    trace_id=trace_id,
                )
                # levibot_exec_orders_total.labels(exchange="mexc", status="blocked", type="market").inc()
                return PaperOrderResult(
                    ok=False, symbol=norm_sym, side=side, qty=0.0, price=0.0, filled=False, pnl_usd=0.0
                )
            ...
            return PaperOrderResult(
                ok=False, symbol=symbol, side=side, qty=0.0, price=0.0, filled=False, pnl_usd=0.0
            )
...
def create_mexc_executor() -> MexcExecutor | None:
    ...
    if not api_key or not api_secret:
        log_event("MEXC_CONFIG_MISSING", {"warning": "MEXC_API_KEY or MEXC_API_SECRET not set"})
        return None
