...
def format_discord(alert: dict[str, Any]) -> dict[str, Any]:
    ...
    embed = {
        ...
        "footer": {"text": f"Source: {source} | Severity: {severity.upper()}"},
    }
...
def route_targets(env: dict[str, str] | None = None) -> list[str]:
    ...
    if default_targets:
        ...
        targets = [
            t
            for t in explicit
            if (t == "slack" and slack_url)
            or (t == "discord" and discord_url)
            or (t == "telegram" and telegram_bot_token and telegram_alert_chat)
        ]
...
def deliver_alert_via(target: str, alert: dict[str, Any], queue) -> None:
    ...
    if target == "slack":
        ...
        try:
            ...
            payload = {
                "text": f"{alert.get('title', 'Alert')}: {alert.get('summary', '')}"
            }
        ...
        try:
            ...
            # Fallback to plain text
            payload = {
                "content": f"{alert.get('title', 'Alert')}: {alert.get('summary', '')}"
            }
