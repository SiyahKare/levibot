...
def run_tick(user_id: str = "default") -> None:
    cfg = load_cfg().get("perp_breakout") or load_cfg().get("strategy", {}).get(
        "perp_breakout", {}
    )
    ...
    for symbol in cfg.get("symbols", ["ETHUSDT"]):
        ...
        go_long = (
            (mark >= rng_hi)
            and (pL >= thr_long)
            and _cooldown_ok(symbol + ":LONG", cooldown)
        )
        go_short = (
            (mark <= rng_lo)
            and (pS >= (1 - thr_short))
            and _cooldown_ok(symbol + ":SHORT", cooldown)
        )
        ...
        pulse_factor = (decision_sig.components.get("telegram_pulse", {}) or {}).get(
            "factor", 1.0
        )
        ...
        try:
            ...
            _last_trade_at[symbol + (":LONG" if side == "buy" else ":SHORT")] = (
                _now_utc()
            )
            log_event(
                "STRAT_OPEN",
            ...
            log_event(
                "ERROR", {"scope": "perp_breakout", "symbol": symbol, "err": str(e)}
            )
...
class _BreakoutTask:
    ...
    def start(self) -> None:
        ...
        def _loop() -> None:
            log_event(
                "BREAKOUT_START", {"task_id": self.id, "params": self.params.__dict__}
            )
            try:
        ...
        self._thread = threading.Thread(
            target=_loop, name=f"perp-breakout-{self.id}", daemon=True
        )
        self._thread.start()
