...
def test_predict_and_log():
    ...
    print(f"✅ Prediction: {pred['symbol']} prob_up={prob_up:.2%} model={pred['model']}")
    ...
    if len(log_data["items"]) > 0:
        ...
        print(f"✅ Latest logged prediction: {latest.get('symbol')} @ {latest.get('model')}")
    else:
...
def test_model_persistence():
    ...
    assert current == target_model, f"Model not persisted: expected {target_model}, got {current}"
