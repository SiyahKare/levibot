...
from backend.src.alerts.queue import WebhookQueue, QueueConfig
...
async def test_queue_retries_and_succeeds():
    ...
    q = WebhookQueue(
        cfg=QueueConfig(rate_limit_per_sec=100, timeout_sec=1.0, max_retries=3, backoff_base=0.05, backoff_max=0.1, jitter=0.0),
...
async def test_rate_limit_blocks_burst():
    ...
    q = WebhookQueue(
        cfg=QueueConfig(rate_limit_per_sec=2, timeout_sec=1.0, max_retries=0, backoff_base=0.01, backoff_max=0.02, jitter=0.0),
        session_factory=lambda: session
    )
    await q.start()
    for _ in range(4):
