/**
 * Daytrade Page - Day Engine Control
 * ─────────────────────────────────────────
 * Intraday trading strategy (15m - 1h timeframes)
 */

import type { DayHealth, DayRunReq } from "@/lib/api";
import { dayHealth, dayPnlStats, dayRun, dayTradesRecent } from "@/lib/api";
...
export default function DaytradePage() {
  ...
  } = useSWR<DayHealth>("/day/health", dayHealth, { refreshInterval: 5000 });

  const {
    data: pnl,
    mutate: refetchPnl,
    isLoading: pnlLoading,
  } = useSWR("/day/pnl/stats?hours=24", () => dayPnlStats(24), {
  ...
  } = useSWR("/day/trades/recent", () => dayTradesRecent(50, true), {
    refreshInterval: 10000,
  });

  const [quoteBudget, setQuoteBudget] = useState(300);
  ...
  const startPaper = async () => {
    ...
    try {
      ...
      await dayRun(req);
      toast.success(`Day Engine started (paper mode, $${quoteBudget}/trade)`);
      setTimeout(() => {
        refetchHealth();
        refetchTrades();
      }, 1000);
    } catch (err: any) {
      console.error("Day start failed:", err);
  ...
  const stopAll = async () => {
    ...
    try {
      ...
      await dayRun(req);
      toast.success("Day Engine soft-stopped");
      setTimeout(() => {
        refetchHealth();
      }, 1000);
    } catch (err: any) {
      console.error("Day stop failed:", err);
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-zinc-600 dark:text-zinc-400 mt-2">
          Intraday trading strategy for medium timeframes (15m - 1h)
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="p-4 rounded-2xl shadow bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-semibold">Day Engine</h2>
            <span
