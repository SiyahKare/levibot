/**
 * Swing Page - Swing Engine Control
 * ─────────────────────────────────────────
 * Multi-day swing trading strategy (4h - 1d timeframes)
 */

import type { SwingHealth, SwingRunReq } from "@/lib/api";
import {
  swingHealth,
  swingPnlStats,
  swingRun,
...
export default function SwingPage() {
  ...
  } = useSWR<SwingHealth>("/swing/health", swingHealth, {
  ...
  } = useSWR("/swing/pnl/stats?hours=168", () => swingPnlStats(168), {
  ...
  } = useSWR("/swing/trades/recent", () => swingTradesRecent(50, true), {
    refreshInterval: 10000,
  });

  const [quoteBudget, setQuoteBudget] = useState(500);
  ...
  const startPaper = async () => {
    ...
    setIsStarting(true);
    try {
      const req: SwingRunReq = {
        mode: "paper",
        overrides: {
          quote_budget: quoteBudget,
          cooldown_bars: 0, // No cooldown for swing
        },
      };
      await swingRun(req);
      toast.success(`Swing Engine started (paper mode, $${quoteBudget}/trade)`);
      setTimeout(() => {
        refetchHealth();
        refetchTrades();
      }, 1000);
    } catch (err: any) {
      console.error("Swing start failed:", err);
  ...
  const stopAll = async () => {
    ...
    try {
      ...
      await swingRun(req);
      toast.success("Swing Engine soft-stopped");
      setTimeout(() => {
        refetchHealth();
      }, 1000);
    } catch (err: any) {
      console.error("Swing stop failed:", err);
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-zinc-600 dark:text-zinc-400 mt-2">
          Multi-day swing trading strategy for long timeframes (4h - 1d)
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="p-4 rounded-2xl shadow bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-semibold">Swing Engine</h2>
            <span
        ...
        <div className="p-4 rounded-2xl shadow bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
          <h3 className="font-semibold mb-2">7d PnL</h3>
          {pnlLoading ? (
            <div className="text-sm text-zinc-500">Loading...</div>
          ) : (
            <>
