...
export default function NFTSniper() {
  ...
  const load = async () => {
    const r = await fetch(`/events?event_type=NFT_SNIPE_CANDIDATE&limit=200`);
    const data: Ev[] = await r.json();
    setRows(
      data.filter(
        (d) =>
          (d.payload?.ask || 0) >= minUsd &&
          (d.payload?.rare_score || 0) >= rare
      )
    );
  };
  useEffect(() => {
    ...
    <div className="p-4">
      ...
      <div className="flex items-center gap-3 mb-3">
        <label className="text-sm">
          Min Ask (USD)
          <input
            className="ml-2 bg-zinc-800 px-2 py-1 rounded w-24"
            type="number"
            value={minUsd}
        ...
        <label className="text-sm">
          ...
          <input
            ...
            step="0.01"
            value={rare}
            onChange={(e) => setRare(Number(e.target.value) || 0)}
          />
        </label>
      </div>
